...
export default function OverallAttendancePage() {
  ...
  const canLoad = Boolean(startDate && endDate && department && typeof year === 'number');
  ...
  const load = async () => {
    ...
    setLoading(true);
    const { data, error } = await supabase.rpc('compute_range_attendance', {
  ...
  const exportCSV = () => {
    ...
    XLSX.writeFile(wb, `attendance_${startDate || 'from'}_${endDate || 'to'}.xlsx`);
  };
        ...
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm">Derived from daily logs across a date range</p>
            </div>
          </div>
        </header>
